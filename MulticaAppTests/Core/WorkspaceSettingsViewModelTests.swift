...
final class WorkspaceSettingsViewModelTests: XCTestCase {
    ...
    private func makeSession() -> AuthSession {
        let session = AuthSession(keychain: KeychainStore(service: "ai.multica.app.workspace-settings.test"), userDefaults: UserDefaults(suiteName: "WorkspaceSettingsViewModelTests.\(UUID().uuidString)")!)
