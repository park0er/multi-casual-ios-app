...
final class LabelsViewModelTests: XCTestCase {
    ...
    func test_missingWorkspaceShowsActionableErrorAndSkipsRequest() async {
        ...
        let vm = LabelsViewModel(api: client, authSession: AuthSession(keychain: KeychainStore(service: "ai.multi-casual.app.labels.empty.test")))
    ...
    private func makeSession() -> AuthSession {
        let session = AuthSession(keychain: KeychainStore(service: "ai.multi-casual.app.labels.test"))
