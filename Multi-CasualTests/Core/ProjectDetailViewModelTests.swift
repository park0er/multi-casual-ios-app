...
final class ProjectDetailViewModelTests: XCTestCase {
    ...
    func test_load_fetchesProjectIssuesAndResources() async throws {
        ...
        let client = makeClient { req in
            ...
            case "/api/projects/p1/resources":
                ...
                {"resources":[{
                    ...
                    "label":"Multi-Casual repo","position":0,"created_at":"2026-01-01T00:00:00Z","created_by":null
                }],"total":1}
                """.data(using: .utf8)!
                return Self.response(for: req, body: json)
            default:
        ...
        XCTAssertEqual(vm.resources.map(\.displayTitle), ["Multi-Casual repo"])
    ...
    func test_load_doesNotRequestProjectDataWhenCurrentWorkspaceDiffersFromProject() async throws {
        ...
        let session = AuthSession(keychain: KeychainStore(service: "ai.multi-casual.app.project-detail.workspace-mismatch.test"))
    ...
    private func makeSession() -> AuthSession {
        let session = AuthSession(keychain: KeychainStore(service: "ai.multi-casual.app.project-detail.test"))
