...
final class AppLanguageSettingsTests: XCTestCase {
    ...
    func test_chineseLanguageLocalizesCoreIssueDetailLabels() {
        ...
        XCTAssertEqual(AppStrings.localized("Add a comment…", language: .zhHans), "添加评论…")
        XCTAssertEqual(AppStrings.localized("Agents Dispatched", language: .zhHans), "已派出 Agents")
    }
    ...
    private func swiftUILiteralKeys(in root: URL) throws -> [String] {
        let sourceDirs = [
            root.appendingPathComponent("Multi-Casual"),
