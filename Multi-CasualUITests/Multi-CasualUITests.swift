...
final class MultiCasualUITests: XCTestCase {
    private let backendTimeoutMessage = "The server took too long to respond. Please try again."
    private let mutationFlagDirectory = URL(fileURLWithPath: "/tmp/multi-casual-ui-mutation-tests", isDirectory: true)
    private let debugTokenFile = URL(fileURLWithPath: "/tmp/multi-casual-ui-debug-token")
    private let debugNetworkLogFile = URL(fileURLWithPath: "/tmp/multi-casual-ui-network-log")

    private var workspaceId: String {
        uiConfigValue(
            environmentKey: "MULTI_CASUAL_UI_WORKSPACE_ID",
    ...
    private var workspaceName: String {
        uiConfigValue(
            environmentKey: "MULTI_CASUAL_UI_WORKSPACE_NAME",
    ...
    private var par73IssueId: String {
        uiConfigValue(
            environmentKey: "MULTI_CASUAL_UI_PAR73_ISSUE_ID",
    ...
    private var projectId: String {
        uiConfigValue(
            environmentKey: "MULTI_CASUAL_UI_PROJECT_ID",
            fileName: "project-id",
    ...
    private var memberUserId: String {
        uiConfigValue(
            environmentKey: "MULTI_CASUAL_UI_MEMBER_USER_ID",
    ...
    private var memberDisplayName: String {
        uiConfigValue(
            environmentKey: "MULTI_CASUAL_UI_MEMBER_DISPLAY_NAME",
    ...
    private var agentDisplayName: String {
        uiConfigValue(
            environmentKey: "MULTI_CASUAL_UI_AGENT_DISPLAY_NAME",
    ...
    private var transcriptTaskId: String {
        uiConfigValue(
            environmentKey: "MULTI_CASUAL_UI_TRANSCRIPT_TASK_ID",
    ...
    func testLoginScreenRendersAndEnablesContinueWithoutSendingCode() {
        ...
        XCTAssertTrue(app.staticTexts["Sign in to Multi-Casual"].waitForExistence(timeout: 20))
    ...
    func testSettingsWorkspaceScreenRenders() throws {
        let app = try launchApp(initialTab: "settings")
    ...
    func testSettingsWorkspacePickerShowsAvailableWorkspace() throws {
        let app = try launchApp(initialTab: "settings")
    ...
    func testSettingsWorkspacePickerSwitchesToAnotherWorkspaceWhenAvailable() throws {
        let app = try launchApp(initialTab: "settings", useWorkspaceOverride: false)
    ...
    func testWorkspaceSwitchRefreshesAlreadyLoadedResourceTabsWhenAvailable() throws {
        let app = try launchApp(initialTab: "issues", useWorkspaceOverride: false)
        ...
        app.terminate()
        let relaunchedApp = try launchApp(initialTab: "settings", useWorkspaceOverride: false)
    ...
    func testSettingsWorkspaceRestoresAcrossRelaunchWithoutDebugWorkspaceOverride() throws {
        let app = try launchApp(initialTab: "settings")
    ...
    func testSettingsAgentsScreenRenders() throws {
        let app = try launchApp(initialTab: "settings")
    ...
    func testSettingsAgentCreateSheetShowsEnvironmentAndArgsEditors() throws {
        let app = try launchApp(initialTab: "settings")
    ...
    func testSettingsAgentCreateEditArchiveRestoreWhenMutationTestsEnabled() throws {
        ...
        guard mutationFlagEnabled(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_AGENT_MANAGEMENT",
            fileName: "agent-management.enabled"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_AGENT_MANAGEMENT=1 or touch /tmp/multi-casual-ui-mutation-tests/agent-management.enabled to mutate a disposable agent.")
        }

        let baseName = mutationValue(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_AGENT_NAME",
            fileName: "agent-name"
    ...
    func testSettingsAutopilotsDataScreenRenders() throws {
        let expectedTitle = ProcessInfo.processInfo.environment["MULTI_CASUAL_UI_AUTOPILOT_TITLE"] ?? ""
        let app = try launchApp(initialTab: "settings")

        XCTAssertTrue(app.staticTexts["Settings"].waitForExistence(timeout: 20))
        tapSettingsRow("Autopilots", in: app)

        XCTAssertTrue(app.staticTexts["Autopilots"].waitForExistence(timeout: 20))
        XCTAssertTrue(app.buttons["AutopilotsNewButton"].waitForExistence(timeout: 10))
        if expectedTitle.isEmpty {
            guard app.cells.firstMatch.waitForExistence(timeout: 20) else {
                throw XCTSkip("No autopilot row is available for data screenshot coverage.")
            }
        } else {
            XCTAssertTrue(staticText(in: app, contains: expectedTitle).waitForExistence(timeout: 30))
        }
        addScreenshot(named: "ios-autopilots-data-via-uitest", from: app)
    }

    func testSettingsApiTokensScreenRenders() throws {
        let app = try launchApp(initialTab: "settings")

        XCTAssertTrue(app.staticTexts["Settings"].waitForExistence(timeout: 20))
        tapSettingsRow("API Tokens", in: app)

        XCTAssertTrue(app.staticTexts["API Tokens"].waitForExistence(timeout: 20))
        XCTAssertTrue(app.textFields["Token name"].waitForExistence(timeout: 10))
        XCTAssertTrue(app.buttons["Create Token"].waitForExistence(timeout: 10))
        XCTAssertTrue(app.staticTexts["Tokens"].waitForExistence(timeout: 10))
        addScreenshot(named: "ios-api-tokens-via-uitest", from: app)
    }
    ...
    func testSettingsRuntimesScreenRenders() {
        ...
        XCTAssertTrue(app.staticTexts["Settings"].waitForExistence(timeout: 20))
        tapSettingsRow("Runtimes", in: app)

        XCTAssertTrue(app.staticTexts["Runtimes"].waitForExistence(timeout: 20))
        addScreenshot(named: "ios-runtimes-via-uitest", from: app)
    }
    ...
    func testSettingsRuntimeDetailRendersWhenRuntimeExists() throws {
        ...
        addScreenshot(named: "ios-runtime-detail-via-uitest", from: app)
    }
    ...
    func testCreateIssueSheetRendersRequiredFields() throws {
        let app = try launchApp(initialTab: "issues", openCreateSheet: true)
    ...
    func testCreateIssueAssigneePickerShowsMembersAndAgents() throws {
        let app = try launchApp(initialTab: "issues", openCreateSheet: true)
    ...
    func testCreateIssuePickersAreTappableFromIssuesTab() throws {
        let app = try launchApp(initialTab: "issues", openCreateSheet: true)
    ...
    func testIssueListFilterSortAndBoardControlsAreReachable() throws {
        let app = try launchApp(initialTab: "issues")
    ...
    func testMyIssuesTabRendersScopesWithoutSubmitting() throws {
        let app = try launchApp(initialTab: "my-issues")
    ...
    func testIssueListBatchSelectionActionsAreReachableWithoutSubmitting() throws {
        let app = try launchApp(initialTab: "issues")
    ...
    func testDemoInteractiveWalkthroughForVideo() throws {
        let language = uiConfigValue(
            environmentKey: "MULTI_CASUAL_UI_DEMO_LANGUAGE",
            fileName: "demo-language",
            defaultValue: "en"
        )
        let localized = DemoLabels(language: language)
        let app = try launchApp(initialTab: "inbox", language: language)
    ...
    func testCreateIssueSubmitsToBackendWhenMutationTestsEnabled() throws {
        ...
        guard mutationFlagEnabled(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_CREATE",
            fileName: "create.enabled"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_CREATE=1 or touch /tmp/multi-casual-ui-mutation-tests/create.enabled to create a real issue.")
        }
    ...
    func testCreateIssueSubmitsMemberProjectDueDateWhenMutationTestsEnabled() throws {
        ...
        guard mutationFlagEnabled(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_CREATE_FIELDS",
            fileName: "create-fields.enabled"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_CREATE_FIELDS=1 or touch /tmp/multi-casual-ui-mutation-tests/create-fields.enabled to create a real issue with extra fields.")
        }

        let title = mutationValue(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_CREATE_TITLE",
        ...
        let app = try launchApp(
            ...
            createDefaults: [
                "MULTI_CASUAL_DEBUG_CREATE_TITLE": title,
                "MULTI_CASUAL_DEBUG_CREATE_STATUS": "todo",
                "MULTI_CASUAL_DEBUG_CREATE_PRIORITY": "high",
                "MULTI_CASUAL_DEBUG_CREATE_ASSIGNEE_OPTION_ID": "member:\(memberUserId)",
                "MULTI_CASUAL_DEBUG_CREATE_PROJECT_ID": projectId,
                "MULTI_CASUAL_DEBUG_CREATE_DUE_DATE": "2026-12-31T00:00:00Z",
            ]
        )
    ...
    func testInboxSwipeShowsArchiveActionWithoutSubmitting() throws {
        let app = try launchApp(initialTab: "inbox")
    ...
    func testInboxMarkAllReadToolbarStateWithoutSubmitting() throws {
        let app = try launchApp(initialTab: "inbox")
    ...
    func testIssueDetailRendersMetadataCommentsAndInput() throws {
        let app = try launchApp(initialTab: "issues", issueId: par73IssueId)
    ...
    func testIssueDetailEditSheetRendersAssignableFields() throws {
        ...
        addScreenshot(named: "ios-edit-issue-via-uitest", from: app)
        assigneePicker.tap()
        XCTAssertTrue(app.buttons["Unassigned"].waitForExistence(timeout: 10))
        XCTAssertTrue(app.buttons[memberDisplayName].waitForExistence(timeout: 10))
        XCTAssertTrue(app.buttons[agentDisplayName].waitForExistence(timeout: 10))
    ...
    func testIssueDetailReassignsIssueWhenMutationTestsEnabled() throws {
        ...
        guard mutationFlagEnabled(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_ISSUE_REASSIGN",
            fileName: "issue-reassign.enabled"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_ISSUE_REASSIGN=1 or touch /tmp/multi-casual-ui-mutation-tests/issue-reassign.enabled to reassign a disposable issue.")
        }
        guard let issueId = mutationValue(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_ISSUE_ID",
            fileName: "issue-id"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_ISSUE_ID or write a disposable issue id to /tmp/multi-casual-ui-mutation-tests/issue-id before reassigning it.")
        }

        let app = try launchApp(initialTab: "issues", issueId: issueId)
    ...
    func testIssueDetailLongCommentsScrollKeepsComposerReachable() throws {
        let app = try launchApp(initialTab: "issues", issueId: par73IssueId)
    ...
    func testIssueListSwipeShowsDoneActionWithoutSubmitting() throws {
        let app = try launchApp(initialTab: "issues")
    ...
    func testIssueListMarksDoneWhenMutationTestsEnabled() throws {
        ...
        guard mutationFlagEnabled(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_ISSUE_STATUS",
            fileName: "issue-status.enabled"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_ISSUE_STATUS=1 or touch /tmp/multi-casual-ui-mutation-tests/issue-status.enabled to update a disposable issue status.")
        }
        guard let issueTitle = mutationValue(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_ISSUE_TITLE",
            fileName: "issue-title"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_ISSUE_TITLE or write a disposable issue title to /tmp/multi-casual-ui-mutation-tests/issue-title before updating status.")
        }
        guard let issueId = mutationValue(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_ISSUE_ID",
            fileName: "issue-id"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_ISSUE_ID or write a disposable issue id to /tmp/multi-casual-ui-mutation-tests/issue-id before updating status.")
        }
        let app = try launchApp(initialTab: "issues")
        ...
        app.terminate()
        let detailApp = try launchApp(initialTab: "issues", issueId: issueId)
    ...
    func testIssueDetailSubmitsCommentWhenMutationTestsEnabled() throws {
        ...
        guard mutationFlagEnabled(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_COMMENT",
            fileName: "comment.enabled"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_COMMENT=1 or touch /tmp/multi-casual-ui-mutation-tests/comment.enabled to post a real comment.")
        }
        guard let issueId = mutationValue(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_ISSUE_ID",
            fileName: "issue-id"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_ISSUE_ID or write a disposable issue id to /tmp/multi-casual-ui-mutation-tests/issue-id before posting a real comment.")
        }
    ...
    func testAgentTranscriptRendersTimeline() throws {
        let app = try launchApp(initialTab: "issues", taskId: transcriptTaskId)
    ...
    func testIssueDetailLiveAgentTimelineUpdatesWhenEnabled() throws {
        ...
        guard mutationFlagEnabled(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_RUNNING_TASK",
            fileName: "running-task.enabled"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_RUNNING_TASK=1 or touch /tmp/multi-casual-ui-mutation-tests/running-task.enabled to observe a real running task.")
        }
        guard let issueId = mutationValue(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_RUNNING_ISSUE_ID",
            fileName: "running-issue-id"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_RUNNING_ISSUE_ID or write a running issue id to /tmp/multi-casual-ui-mutation-tests/running-issue-id.")
        }
    ...
    func testProjectDetailRendersResourcesAndIssues() throws {
        let app = try launchApp(initialTab: "projects", projectId: projectId)

        XCTAssertTrue(app.staticTexts["Multi-Casual iOS App"].waitForExistence(timeout: 20))
    ...
    func testProjectCreateSheetOpensWithoutSubmitting() throws {
        let app = try launchApp(initialTab: "projects")
    ...
    func testChatDetailScreenRendersWhenSessionExists() throws {
        let app = try launchApp(initialTab: "inbox")
        ...
        var firstSessionCell: XCUIElement?
        while Date() < deadline {
            if app.staticTexts["No Chats"].exists {
                throw XCTSkip("No chat session is available for detail screenshot coverage.")
            }
            if app.staticTexts[backendTimeoutMessage].exists {
                throw XCTSkip("Chat endpoint timed out before detail screenshot coverage could run.")
            }
            if let cell = app.cells.allElementsBoundByIndex.first(where: { $0.isHittable }) {
                firstSessionCell = cell
                break
            }
            RunLoop.current.run(until: Date().addingTimeInterval(0.5))
        }

        guard let firstSessionCell else {
            XCTFail("Timed out waiting for a chat session row.")
            return
        }
        let sessionButton = firstSessionCell.buttons.firstMatch
        if sessionButton.exists {
            sessionButton.tap()
        } else {
            firstSessionCell.tap()
        }

        XCTAssertTrue(app.textFields["ChatMessageField"].waitForExistence(timeout: 20))
        XCTAssertTrue(app.buttons["ChatArchiveButton"].waitForExistence(timeout: 10))
        XCTAssertTrue(app.staticTexts["Messages"].waitForExistence(timeout: 10))
        addScreenshot(named: "ios-chat-detail-via-uitest", from: app)
    }
    ...
    func testInboxMarksReadAndArchivesWhenMutationTestsEnabled() throws {
        ...
        guard mutationFlagEnabled(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_INBOX_ACTIONS",
            fileName: "inbox-actions.enabled"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_INBOX_ACTIONS=1 or touch /tmp/multi-casual-ui-mutation-tests/inbox-actions.enabled to mutate a real Inbox item.")
        }
    ...
    private func launchApp(
        ...
        app.launchEnvironment["MULTI_CASUAL_DEBUG_SKIP_PUSH_PROMPT"] = "1"
        if ProcessInfo.processInfo.environment["MULTI_CASUAL_UI_DEBUG_NETWORK_LOG"] == "1"
            || FileManager.default.fileExists(atPath: debugNetworkLogFile.path) {
            app.launchEnvironment["MULTI_CASUAL_DEBUG_NETWORK_LOG"] = "1"
        }
        if let token = debugToken() {
            app.launchEnvironment["MULTI_CASUAL_DEBUG_TOKEN"] = token
        } else {
            throw XCTSkip("Authenticated UI tests require MULTI_CASUAL_UI_DEBUG_TOKEN in the test runner environment.")
        }
        if useWorkspaceOverride {
            app.launchEnvironment["MULTI_CASUAL_DEBUG_WORKSPACE_ID"] = workspaceId
        }
        app.launchEnvironment["MULTI_CASUAL_DEBUG_APP_LANGUAGE"] = language
        app.launchEnvironment["MULTI_CASUAL_DEBUG_INITIAL_TAB"] = initialTab
        if let issueId {
            app.launchEnvironment["MULTI_CASUAL_DEBUG_INITIAL_ISSUE_ID"] = issueId
        }
        if let projectId {
            app.launchEnvironment["MULTI_CASUAL_DEBUG_INITIAL_PROJECT_ID"] = projectId
        }
        if let taskId {
            app.launchEnvironment["MULTI_CASUAL_DEBUG_INITIAL_TASK_ID"] = taskId
        }
        if openCreateSheet {
            app.launchEnvironment["MULTI_CASUAL_DEBUG_OPEN_CREATE_SHEET"] = "1"
        }
    ...
    private func debugToken() -> String? {
        if let token = ProcessInfo.processInfo.environment["MULTI_CASUAL_UI_DEBUG_TOKEN"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !token.isEmpty {
            return token
        }
    ...
    private func launchLoggedOutApp() -> XCUIApplication {
        let app = XCUIApplication()
        app.launchEnvironment["MULTI_CASUAL_DEBUG_SKIP_PUSH_PROMPT"] = "1"
        app.launchEnvironment["MULTI_CASUAL_DEBUG_FORCE_LOGIN_SCREEN"] = "1"
        addTeardownBlock {
            if app.state != .notRunning {
                app.terminate()
            }
    ...
    private func launchStubbedAuthenticatedApp(initialTab: String, language: String? = nil) -> XCUIApplication {
        let app = XCUIApplication()
        app.launchEnvironment["MULTI_CASUAL_DEBUG_SKIP_PUSH_PROMPT"] = "1"
        app.launchEnvironment["MULTI_CASUAL_DEBUG_AUTH_STUB"] = "1"
        app.launchEnvironment["MULTI_CASUAL_DEBUG_INITIAL_TAB"] = initialTab
        app.launchEnvironment["MULTI_CASUAL_DEBUG_WORKSPACE_ID"] = workspaceId
        app.launchEnvironment["MULTI_CASUAL_DEBUG_WORKSPACE_NAME"] = workspaceName
        app.launchEnvironment["MULTI_CASUAL_DEBUG_WORKSPACE_SLUG"] = workspaceName
        app.launchEnvironment["MULTI_CASUAL_DEBUG_WORKSPACE_PREFIX"] = "PAR"
        app.launchEnvironment["MULTI_CASUAL_DEBUG_APP_LANGUAGE"] = language ?? "en"
        addTeardownBlock {
            if app.state != .notRunning {
                app.terminate()
            }
    ...
    private func requireMutationTestsEnabled(reason: String) throws {
        guard mutationFlagEnabled(
            environmentKey: "MULTI_CASUAL_UI_MUTATION_TESTS",
            fileName: "enabled"
        ) else {
            throw XCTSkip("Set MULTI_CASUAL_UI_MUTATION_TESTS=1 in the Xcode test runner, or touch /tmp/multi-casual-ui-mutation-tests/enabled, to \(reason).")
        }
    }
